...
func ResourceChange(
	...
	case plans.Delete:
		...
		case plans.ResourceInstanceDeleteBecauseNoResourceConfig:
			buf.WriteString("\n  # (because resource was removed from configuration)")
		case plans.ResourceInstanceDeleteBecauseNoModule:
			buf.WriteString(fmt.Sprintf("\n  # (because %s was removed from configuration)", addr.Module))
		case plans.ResourceInstanceDeleteBecauseWrongRepetition:
			// We have some different variations of this one
			switch addr.Resource.Key.(type) {
			case nil:
				buf.WriteString("\n  # (because resource now uses count or for_each)")
			case addrs.IntKey:
				buf.WriteString("\n  # (because resource no longer uses count)")
			case addrs.StringKey:
				buf.WriteString("\n  # (because resource no longer uses for_each)")
			}
		case plans.ResourceInstanceDeleteBecauseCountIndex:
			buf.WriteString(fmt.Sprintf("\n  # (because instance %s is out of range for count)", addr.Resource.Key))
	...
	if change.Moved() && change.Action != plans.NoOp {
		buf.WriteString(fmt.Sprintf(color.Color("  # [reset](%s has moved to %s)\n"), change.PrevRunAddr.String(), dispAddr))
	}
